# Car-Analysis-Dashboard
The dataset is a comprehensive car sales dataset sourced from Kaggle (https://www.kaggle.com/datasets/safaeahb/car-sales-analysis-dashboard?resource=download&select=car+sales.csv) and
consists of 23,906 records representing detailed car sales transactions from 2022 to
2023. It contains 17 attributes encompassing various dimensions of the car sales
process, including customer demographics, vehicle specifications, sales pricing, and
dealership details.

## Dashboard 1
<img width="762" height="419" alt="image" src="https://github.com/user-attachments/assets/57dfa4a3-b679-4634-b3cf-ff4bcef12e50" />

## Dashboard 2 
<img width="760" height="421" alt="image" src="https://github.com/user-attachments/assets/9855db22-0dee-49c6-886d-1a54cf9a467c" />

